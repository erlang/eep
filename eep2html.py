...
EEP, see http://www.erlang.org/eeps/eep-0001.html for instructions and links
...
def linkemail(address, pepno):
    parts = address.split('@', 1)
    return ('<a href="mailto:%s&#64;%s?subject=EEP%%20%s">'
            '%s&#32;&#97;t&#32;%s</a>'
...
def fixfile(inpath, input_lines, outfile):
    ...
    if basename <> 'eep-0000.txt':
        print >> outfile, '[<b><a href=".">EEP Index</a></b>]'
    if pep:
        try:
            print >> outfile, ('[<b><a href="eep-%04d.txt">EEP Source</a>'
                               '</b>]' % int(pep))
        except ValueError, error:
            print >> sys.stderr, ('ValueError (invalid EEP number): %s'
                                  % error)
    ...
    for k, v in header:
        if k.lower() in ('author', 'discussions-to'):
            ...
            for otherpep in re.split(',?\s+', v):
                ...
                otherpeps += '<a href="eep-%04d.html">%i</a> ' % (otherpep,
                                                                  otherpep)
            v = otherpeps
...
def get_pep_type(input_lines):
    ...
    Return ``None`` if the input is not a EEP.
    """
    pep_type = None
...
def make_html(inpath, verbose=0):
    ...
    if pep_type is None:
        ...
        print >> sys.stderr, ('Error: Unknown EEP type for input file %s: %s'
                              % (inpath, pep_type))
        sys.stdout.flush()
        return None
...
def check_requirements():
    ...
    try:
        ...
        PEP_TYPE_MESSAGES['text/plain'] = (
            'Python %s or better required for "%%(eep_type)s" EEP '
    ...
    try:
        ...
        PEP_TYPE_MESSAGES['text/x-rst'] = (
            'Docutils not present for "%(eep_type)s" EEP file %(inpath)s.  '
