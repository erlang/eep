...
Specification
    ...
    (a) A portable fun may not contain plain funs.
    (b) A portable fun may not contain a call f(...)
        without a module prefix unless f is a built-in
        function.
    (c) A portable fun may not contain any call of the
        form M:f(...) or m:F(...) or M:F(...).
    (d) A portable fun may not contain any call of the
        form F(...) unless F is bound in its head.
    (e) In a system where abstract patterns are available,
    ...
    (2) In a {module,Module} item for a portable fun, the Module
        ...
        connection between a portable fun and any module by that
        name.
    ...
    (5) {calls,List} will be returned for a portable fun,
        ...
        need loading and which functions they are expected to
        export.
    (6) For consistency,
    ...
    erlang:is_portable_function(Term)
...
Reference Implementation
    ...
    (A) a fun representation that holds instructions in a binary
    ...
    (B) a compilation strategy for comprehensions that, like the
