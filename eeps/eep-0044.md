...
We will introduce a new predefined macro and three new preprocessor
directives.
...
allowed in guards, with two differences:
...
- `version(`*App*`)` returns the version number for the given app
*App* as a tuple of three integers.  The version number will be read
from the app file.  For example, `version(compiler)` could return
`{6,0,0}`.  If the application does not exist or no version number is
found in the app file, `{0,0,0}` will be returned.
...
specifications for all exported functions to help find errors.
...
example, code that uses the map syntax).  Therfore, it seems that
