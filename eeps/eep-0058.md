...
Maps and initial map comprehension syntax and semantics were proposed
in EEP-43. List and binary comprehensions were specified post-fact
in EEP-12. This EEP is based on those two EEPs.

Syntax construct commonly referred as "comprehension" is in fact
consists of 3 semi-independent parts:

- "generator" - is the expression that iterates over the container,
  matches each element of the container over pattern and binding
  variables which can be used in "comprehensions" and "filters".
- "comprehension" - is the expression that is executed for each
  element produced by generator that successfully matched the pattern
  and passed the "filter" check. Value produced by this expression
  is appended to the resulting generated container.
- "filter" - is the expression that is evaluated for each element
  produced by generator that matched the pattern and returns a
  boolean result: when it returns `false` the current element is
  skipped from being passed to "comprehension"; when it results
  in `true`, the element is passed to "comprehension".

So, in this document we are going to keep this separation, but
sometimes calling the whole combination of comprehension, filter and
generator as just "comprehension", because generator syntax is not
used outside of the comprehension context.


### Syntax

Proposed syntax for map comprehension part is

    '#{' KeyExpr '=>' ValueExpr '||' GeneratorsFilters '}'

Where `GeneratorsFilters` are one or more list/map/binary generator
and/or filter, separated by comma.

Proposed syntax for map generator part is
...
`maps:from_list/1` call over the list comprehension producing
the list of 2-tuples:
...
generator, there is no need for a new arrow. `<-` were chosen over
`<=` because latter is less visually distinguishable when it is
...
Since maps do not have any order specified, the order in which
...
Same as `maps:from_list/1`: the latter (right-most) value is used
and the previous values are ignored. It might be less predictable
when map comprehension is used in combination with map generator
(because map generator order is not defined), but is important when
map comprehension is used with list of binary generator.
...
One (probably minor) argument against is the fact that "map update"
syntax is a common reason of very confusing errors programmers make
...
Here "map update" syntax have been accidentally created instead of
intended "list of two maps".
...
seen in a wild to use combinations of `maps:from_list/1`,
