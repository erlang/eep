...
in the [Sigil][] value cannot be allowed to affect e.g `re:run/3`
that has options as the third argument.  If `re:run/3` would fail
for any compile options (only allow runtime options), or if
...
since the running Erlang code *must* see a compiled regulare expression.
...
is possible with that, this EEP suggests that a regular expression
Sigil should represent an *uncompiled* regular expression,
with compile flags.
