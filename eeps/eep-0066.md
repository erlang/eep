...
* «»: the Vanilla [Sigil][].
  ...
  Creates a `unicode:unicode_binary()` in Erlang.
...
* «`r`»: regular expression.
  ...
  suitable as the `Regexp` argument to functions in the `re` module.
  The `RE` element is the [String Content][],
  and the `Flags` element is the [Sigil Suffix][].

  The `re` module needs to be augmented with functions that
  accept this tuple format containing both a regular expression,
  and compile options in the guise of character flags.
  Translation of the character `Flags` into `re:compile_option()`s
  should be done by these `re` module functions.

  Within the [String Content][], character escape sequences
  are handled according to the regular expression rules,
  and in addition to them the [end delimiter][4] can be escaped
  with a «`\`» character.  Within triple-quote delimiters
  according to [EEP 64][], end delimiter escaping is not done.
...
which may affect finding the [end delimiter][4].
