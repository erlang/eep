...
unsuccesful matches within the same construct without risking to confuse
...
Since the we force a return on any non-matching value, the whole expression, if
...
obscure its origin and line nubmer.
...
These problems would *not* exist with normalized error and return values as those used in statically-typed languages, but since we do not intend to normalize values, the `else` blog is a necessary workaround.
