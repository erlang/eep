...
class Headers(Transform):
    ...
    def apply(self):
            ...
            if field[0].astext().lower() == 'eep': # should be the first field
                ...
                try:
                    ...
                    msg = self.document.reporter.warning(
                        '"EEP" header must contain an integer; "%s" is an '
        ...
        for field in header:
            ...
            body = field[1]
            if len(body) > 1:
                raise DataError('EEP header field body contains multiple '
                                'elements:\n%s' % field.pformat(level=1))
            elif len(body) == 1:
                if not isinstance(body[0], nodes.paragraph):
                    raise DataError('EEP header field body may only contain '
                                    'a single paragraph:\n%s'
...
def mask_email(ref, pepno=None):
    ...
    if ref.hasattr('refuri') and ref['refuri'].startswith('mailto:'):
        ...
        if pepno is None:
            ...
            ref['refuri'] += '?subject=EEP%%20%s' % pepno
            ref[:] = [replacement]
            return ref
    else:
        return ref
